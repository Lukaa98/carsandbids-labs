(() => {
  ...
  class PanelUI {
    ...
    mount(){
      ...
      const onMove=(e)=>{ if(!resizing) return; const dx=e.clientX-startX, dy=e.clientY-startY;
    ...
    setBody(text){ if(this.bodyNode) this.bodyNode.textContent = text; }
    setBodyBlocks(blocks){ if(this.bodyNode) this.bodyNode.textContent = blocks.join("\n\n"); }
    setMeta(text){ if(this.metaNode) this.metaNode.textContent = text; }
  }
